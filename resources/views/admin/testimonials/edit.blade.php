...
@section('title', 'Fikrni tahrirlash')

@section('content')
    <div class="p-6 space-y-6">
        <h1 class="text-3xl font-bold text-gray-900">Fikrni tahrirlash</h1>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="course" id="course"
                            value="{{ old('course', $testimonial->course) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm"
                    ...
                    <div>
                        ...
                        <textarea name="comment" id="comment" rows="4"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm"
                            placeholder="Fikrni kiriting" required>{{ old('comment', $testimonial->comment) }}</textarea>
                        @error('comment')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
