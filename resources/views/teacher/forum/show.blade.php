                ...
                <div class="rounded-lg border bg-white shadow-sm p-6">
                    @forelse($thread->posts as $post)
                        <div class="py-4 border-b border-gray-200 last:border-b-0">
                            ...
                            @if($post->user_id === Auth::id() || Auth::user()->role === 'teacher')
                                <form action="{{ route('teacher.forum.post.destroy', [$course->id, $thread->id, $post->id]) }}"
                    ...
                    <form action="{{ route('teacher.forum.post.store', [$course->id, $thread->id]) }}" method="POST"
                        class="mt-6">
                        @csrf
                        <div class="flex flex-col gap-2">
                            <textarea name="content" class="rounded-md border-gray-300" rows="4"
                                placeholder="Javobingizni kiriting" required></textarea>
                            <button type="submit"
